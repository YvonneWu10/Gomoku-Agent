...
from policy_value_net_gabr import PolicyValueNet
...
class Game(object):
    ...
    def graphic(self, board, player1, player2):
        ...
        print("Player", player1, "with X".rjust(3), flush=True)
        print("Player", player2, "with O".rjust(3), flush=True)
        print(flush=True)
        for x in range(width):
            print("{0:8}".format(x), end='', flush=True)
        print('\r\n', flush=True)
        for i in range(height - 1, -1, -1):
            print("{0:4d}".format(i), end='', flush=True)
            for j in range(width):
                loc = i * width + j
                p = board.states.get(loc, -1)
                if p == player1:
                    print('X'.center(8), end='', flush=True)
                elif p == player2:
                    print('O'.center(8), end='', flush=True)
                else:
                    print('_'.center(8), end='', flush=True)
            print('\r\n\r\n', flush=True)
    ...
    def start_play(self, player1, player2, start_player=0, is_shown=1):
        ...
        while True:
            ...
            if end:
                if is_shown:
                    if winner != -1:
                        print("Game end. Winner is", players[winner], flush=True)
                    else:
                        print("Game end. Tie", flush=True)
                return winner
    ...
    def start_self_play(self, player, is_shown=0, temp=1e-3):
        ...
        while True:
            ...
            if end:
                ...
                if is_shown:
                    if winner != -1:
                        print("Game end. Winner is player:", winner, flush=True)
                    else:
                        print("Game end. Tie")
    ...
    def policy_evaluate(self, model_file, n_games=10):
        ...
        best_policy = PolicyValueNet(self.board.width, self.board.height, res_block_num=5)
        ...
        for i in range(n_games):
            winner = self.start_play(current_mcts_player, pure_mcts_player, start_player=i % 2, is_shown=1)
            win_cnt[winner] += 1
            print(f"game {i + 1}: winner is {winner}", flush=True)
        win_ratio = 1.0*(win_cnt[1] + 0.5*win_cnt[-1]) / n_games
        print("num_playouts:{}, win: {}, lose: {}, tie:{}".format(
                self.pure_mcts_playout_num,
                win_cnt[1], win_cnt[2], win_cnt[-1]), flush=True)
        return win_ratio
