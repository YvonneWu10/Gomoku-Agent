...
from policy_value_net_withGAP_single import PolicyValueNet as PolicyValueNetGap
...
class Board(object):
    ...
    def get_all_movements(self):
        ...
        for i in range(0,2):
            ...
            if i % 2 ==0:
                move['first'].append(location)
            else:
                move['second'].append(location)
        return movements
...
class Game(object):
    ...
    def start_play(self, player1, player2, start_player=0, is_shown=1):
        ...
        # p1_player_execution_time=[]
        # p2_player_execution_time=[]
        if is_shown:
        ...
        while True:
            ...
            #     p1_player_execution_time.append(execution_time)
            # else:
            #     p2_player_execution_time.append(execution_time)
            ...
            if end:
                if is_shown:
                    if winner != -1:
                        ...
                        print("Game end. Tie")
                # p1_average = sum(p1_player_execution_time)/len(p1_player_execution_time)
                # p2_average = sum(p2_player_execution_time)/len(p2_player_execution_time)
                # print("p1执行时间平均值：",p1_average)
                # print("p2执行时间平均值：",p2_average)
                return winner
    ...
    def policy_evaluate(self, model_file, n_games=10):
        ...
        """
        # origin_best_policy =PolicyValueNetOrigin(self.board.width,self.board.height)
        # origin_best_policy.policy_value_net.load_state_dict(torch.load('loey/goproject/gpoolsingle/origin_best_policy.model'))
        # origin_mcts_palyer = MCTSPlayer(origin_best_policy.policy_value_fn,c_puct=5,n_playout=200)
        ...
        for i in range(n_games):
            #winner = self.start_play(current_mcts_player, origin_mcts_palyer, start_player=i % 2, is_shown=0)
            winner = self.start_play(current_mcts_player, pure_mcts_player, start_player=i % 2, is_shown=0)
            #winner = self.start_play(current_mcts_player, mcts_player_200, start_player=i % 2, is_shown=0)
            win_cnt[winner] += 1
            print(winner)
    ...
    def play(self,players,click):
        # Returns:win,winner,movements:win or draw,winner of game,movements.
        current_player = self.board.get_current_player()
        player_in_turn = players[current_player]
        if player_in_turn.id == 'ai':
            move = player_in_turn.get_action(self.board)
        else:
            move = player_in_turn.get_action(self.board,click)
        #performance a move
        self.board.do_move(move)
        end, winner = self.board.game_end()
        movements = self.board.get_all_movements()
        return end,winner,movements
...
if __name__ == '__main__':
    ...
    board_height = 9
    n_in_row = 5
    model_file = "GP_model_7000.model"
